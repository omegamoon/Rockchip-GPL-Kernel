			    ...
			    u16 size, void *data, int eflag)
{
	...
	if (eflag && (2 == size)) {
		u16 buf_le;
		ret = __ax88179_read_cmd(dev, cmd, value, index, size, &buf_le, 0);
		le16_to_cpus(&buf_le);
		*((u16 *)data) = buf_le;
	} else if (eflag && (4 == size)) {
		u32 buf_le;
...
ax88179_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags)
{
	...
	/* 2 tx_hdr headers need 4+4 bytes plus the misalignment correction space 3 bytes */
	need_room = 8 + 3;
	
	if ((headroom + tailroom) >= need_room) {
		if (headroom < need_room) {
			/* skb->data should be 4-byte aligned, calculate the misalignment */
			head_align = (4 - ((uintptr_t)skb->head & 3)) % 4;
			/* calculate room needed, so data will be aligned */
			need_room = 8 + head_align;
			
			skb->data = memmove(skb->head + need_room, skb->data, skb->len);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 22)
	...
	if (data_align > 0) {
		skb->data = memmove(skb->data - data_align, skb->data, skb->len);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 22)
			skb->tail = skb->data + skb->len;
#else
			skb_set_tail_pointer(skb, skb->len);
