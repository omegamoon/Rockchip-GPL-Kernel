...
/* Logging */
#define IPP_DEBUG 1  // <<< Omegamoon - Enable debug info
#if IPP_DEBUG
...
static void ipp_blit_complete(int retval)
{
	...
	DBG("Omegamoon >>> IPP Blit Complete\n");
...
static int ipp_get_result(unsigned long arg)
{
	...
	DBG("Omegamoon >>> IPP get_result %d\n", drvdata->ipp_result);
...
{
	int ret = -1;
	DBG("Omegamoon >>> IPP Blit Async Start\n");
	...
	DBG("Omegamoon >>> IPP Blit Async Finished\n");
	return ret;
}
